...
def export_web_json(
    ...
    # JS bundle — loads without fetch() (more reliable on GitHub Pages)
    js_path = out / "dashboard-data.js"
    js_blob = (
        "/* auto-generated by pipeline.export_web — do not edit */\n"
        "window.__OLIST_DASHBOARD__ = "
        + body
        + ";\n"
    )
    ...
    # Mirror into js/ for GitHub Pages (Jekyll sometimes mishandles assets/*)
    js_dir = REPO_ROOT / "js"
    js_dir.mkdir(parents=True, exist_ok=True)
    mirror_js = js_dir / "dashboard-data.js"
    mirror_json = js_dir / "dashboard.json"
    with open(mirror_js, "w", encoding="utf-8") as f:
        f.write(js_blob)
    with open(mirror_json, "w", encoding="utf-8") as f:
        json.dump(payload, f, ensure_ascii=False, indent=2, default=_json_default)
    ...
    logger.info("Web export → %s + %s", js_path, mirror_js)
